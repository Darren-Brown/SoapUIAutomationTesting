...
using IQ.Platform.Framework.WebApi.MediaTypeFormatters;
using IQ.Platform.Framework.WebApi.Model.Hypermedia;
using HypemediaWebApi2012.Documentation.Controllers;
...
namespace HypemediaWebApi2012.WebApi.Infrastructure
{
    public static class BootStrapper
    {
        ...
        static void InitializeDocumentationApplication(HttpConfiguration config, IApiContainer container)
        {
            ...
            const string apiName = "HypemediaWebApi2012 API documentation";
        ...
        static void ConfigureFormatters(HttpConfiguration config, IApiContainer container)
        {
            ...
            // Remove the default Xml and Json formatter
            config.Formatters.Remove(config.Formatters.XmlFormatter);
            config.Formatters.Remove(config.Formatters.JsonFormatter);

            // Add the custom formatters
            config.Formatters.Add(container.ResolveMediaTypeFormatter<CustomXmlMediaTypeFormatter>());
            config.Formatters.Add(container.ResolveMediaTypeFormatter<CustomJsonMediaTypeFormatter>());
